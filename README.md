...
raft-l2-oracle/
  firmware/
    ...
    svd/STM32F217.svd              # Peripheral register definitions for debugger (F217 superset of F207)
  ...
  .vscode/
    extensions.json                # Recommended extensions (Cortex-Debug, clangd, CMake Tools)
    launch.json                    # Debug/flash/attach configs for OpenOCD + ST-LINK
    settings.json                  # clangd + CMake Tools configuration
...
The repo includes `.vscode/` configuration for colleagues who prefer a graphical IDE. Install the recommended extensions when prompted (Cortex-Debug, clangd, CMake Tools), then:

- **Build**: `Ctrl+Shift+B` (default task: Build Firmware)
- **Debug**: `F5` selects "Debug Firmware (OpenOCD)" - builds, flashes, and breaks at `main()`
- **Flash and run**: Use the "Flash and Run" launch config to program and let the firmware run freely
- **Attach**: "Attach to Running Firmware" connects to an already-running board without reflashing
- **Peripheral registers**: The SVD file (`firmware/svd/STM32F217.svd`) enables the Cortex-Debug peripheral register viewer. STM32F217 is used because no F207-specific SVD exists - F217 is a register-compatible superset (adds only CRYP/HASH).
- **Presets**: Switch between firmware (ARM cross-compile) and simulator (native) via the CMake Tools status bar

clangd provides IntelliSense using the `compile_commands.json` generated by CMake. The `--query-driver` setting handles arm-none-eabi system includes automatically regardless of toolchain install path.

These files do not affect terminal or CLion workflows - they are ignored by all non-VS-Code tooling.
